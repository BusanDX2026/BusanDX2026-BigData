...
# T03 검증 — 수치는 M9 가 내보낸 04_모델/M9_검증.json 에서 읽는다.
#   ⚠ 하드코딩 금지 (이슈 #43). 사건 검증은 "학습 제외"가 아니라 라벨 1→0 민감도다.
_V = json.load(open(GG / "04_모델" / "M9_검증.json", encoding="utf-8"))
_H = {r["event"]: r for r in _V["holdouts"]}
_HZ = {r["flag"]: r for r in _V["hazdist"]}
_t3 = [["공간 교차검증", "자치구 GroupKFold 5-fold",
        "PR-AUC %.4f" % _V["pr_auc"],
        "랜덤 K-fold(0.5189)는 +70% 과대평가 — 공간 자기상관 제거"]]
for _e, _why in [("2011-07-27", "동일"),
                 ("2020 대형호우", "평범한 대형호우는 잘 일반화"),
                 ("2014-08-25", "미증유 극한은 평상시 안전지까지 침수 — 전면대응 국면")]:
    _r = _H[_e]
    _t3.append(["사건 라벨 민감도", "%s 라벨 1→0 (격자는 학습셋 유지)" % _e,
                "리프트 %.1f배 · 상위10%% %.1f%% 포착" % (_r["lift"], _r["top10"] * 100),
                _why])
_z = _HZ["hazdist_flag"]
_t3 += [
    ["기준선 비교", "행정 재해위험지구 (동일 격자 수 %d개, OOF)" % _z["k"],
     "정밀도 %.1f%% → %.1f%%" % (_z["base"] * 100, _z["model"] * 100),
     "같은 면적을 점검하면 %.2f배 더 포착. 침수흔적의 %.1f%%가 지정구역 밖" % (_z["ratio"], _V["outside_ratio"] * 100)],
    ["기준선 비교", "무작위 / MCDA 동일가중", "10.2% / 7.7%", "물리지표 단순 평균은 무작위보다 나쁨"],
    ["편향 통제", "자치구 내부 순위상관", "Spearman 중앙값 +0.27", "구 내부에서도 양의 관계 (편향 해소를 입증하는 것은 아님)"],
    ["변수 선정", "행정동 난수 대조 · 변수 제거 비교 · SHAP 방향성", "지하차도 −13.2% (위약 −11.7%)",
     "기여도가 높아도 일반화·물리 타당성이 부족하면 제외 (M8 맨홀 4종, M11 지하차도)"],
    ["독립 출처", "119 배수출동 (인구 통제)", "M9 ρ=+0.201 (p=0.040)", "다른 출처에서도 물리 위험도 신호 유의"],
]
save(pd.DataFrame(_t3, columns=["검증 유형", "방법", "결과", "해석"]), "T03_검증요약", "검증 수행 결과")
