...
FIGS = [
    ...
    ("F08_홀드아웃검증.png", "그림 8", "사건 홀드아웃 검증",
     "특정 호우 격자의 라벨만 1→0 으로 바꿔(격자는 학습셋 유지) 그 침수를 맞히는지 "
     "시험했다. 평범한 대형호우는 "
...
with PdfPages(PDF) as pdf:
    ...
    for lab, val in [("행정 재해위험지구 대비 (동일 면적·OOF)", "3.08배  (17.0% → 52.5%)"),
                     ("2020 대형호우 라벨 민감도", "16.8배 — 라벨을 지워도 다시 찾아냄"),
